...
# ---------------------------------------------------------------------------
# Health check (for keep-alive pings, not part of the MCP protocol itself)
# ---------------------------------------------------------------------------
# Hit GET {teamUrl}/health every 2-3 minutes from an external cron/uptime
# pinger to stop the platform's container from going idle/cold. This route
# does nothing but return 200 instantly, so it never touches the real tools
# or counts against the evaluation rate limit.
...
if __name__ == "__main__":
    ...
    # env var and route traffic ONLY to that port -- hardcoding 8000 works
    # locally but silently breaks on Render, since nothing is listening on
    # the port Render actually forwards to. Falls back to 8000 for local runs.
